...
def _encode_version_prefix(version: int) -> bytes:
    """Encode a Base58 address version prefix.

    Most networks use a single version byte, but some (for example Zcash
    transparent addresses) use a two-byte big-endian prefix such as 0x1CB8.
    Single-byte values (<= 0xFF) are unaffected.
    """
    if version < 0:
        raise AddressError("Version prefix must be non-negative.")
    if version <= 0xFF:
        return bytes([version])
    length = (version.bit_length() + 7) // 8
    return version.to_bytes(length, "big")
...
class Bech32:
    ...
    def _verify_checksum(cls, hrp: str, data: List[int]) -> Optional[str]:
        """Return the matching spec ('bech32'/'bech32m') or None if invalid."""
        polymod = cls._polymod(cls._hrp_expand(hrp) + data)
        if polymod == cls.BECH32_CONST:
            return "bech32"
        if polymod == cls.BECH32M_CONST:
            return "bech32m"
        return None

    @classmethod
    def decode(cls, address: str) -> Tuple[str, List[int], str]:
        """Decode a Bech32/Bech32m string into (hrp, data, spec).

        The 6-symbol checksum is verified and stripped. Raises AddressError on
        ...
        if any(ord(char) < 33 or ord(char) > 126 for char in address):
            raise AddressError("Address contains non-printable characters.")
        if address.lower() != address and address.upper() != address:
            raise AddressError("Address must not mix upper and lower case.")
        ...
        for char in data_part:
            if char not in cls.CHARSET:
                raise AddressError(f"Invalid Bech32 data character: '{char}'")
            data.append(cls.CHARSET.index(char))

        spec = cls._verify_checksum(hrp, data)
        if spec is None:
            raise AddressError("Invalid Bech32 checksum.")
        return hrp, data[:-6], spec

    @classmethod
    def decode_witness(cls, address: str) -> Tuple[str, int, bytes]:
        """Decode a SegWit address into (hrp, witness_version, program)."""
        hrp, data, spec = cls.decode(address)
        if not data:
        ...
        program_ints = cls.convert_bits(data[1:], 5, 8, False)
        if program_ints is None:
            raise AddressError("Invalid witness program padding.")
        program = bytes(program_ints)

        if witness_version == 0:
            if len(program) not in (20, 32):
                raise AddressError("Witness version 0 program must be 20 or 32 bytes.")
            if spec != "bech32":
                raise AddressError("Witness version 0 must use Bech32 encoding.")
        else:
            if not 2 <= len(program) <= 40:
                raise AddressError("Witness program must be 2..40 bytes.")
            if spec != "bech32m":
                raise AddressError("Witness version 1..16 must use Bech32m encoding.")
...
class AddressGenerator:
    ...
    def _generate_bitcoin_style_address(cls, public_key: bytes, address_type: str, network: str) -> str:
        ...
        if address_type in {"p2pkh", "legacy"}:
            return base58_check_encode(_encode_version_prefix(versions["p2pkh"]) + key_hash)

        if address_type in {"p2sh-p2wpkh", "p2sh_p2wpkh", "nested-segwit", "nested_segwit"}:
            redeem_script = b"\x00\x14" + key_hash
            script_hash = hash160(redeem_script)
            return base58_check_encode(_encode_version_prefix(versions["p2sh"]) + script_hash)
