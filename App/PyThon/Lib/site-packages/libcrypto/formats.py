...
def base58_encode(data: bytes, alphabet: str = BASE58_ALPHABET) -> str:
    ...
    # Accumulate into a list and reverse once to avoid quadratic string joins.
    chars = []
    while num > 0:
    ...
    return alphabet[0] * leading_zeros + "".join(chars)
...
def base58_decode(encoded: str, alphabet: str = BASE58_ALPHABET) -> bytes:
    ...
    if not encoded:
        return b""
    # A char->value map turns each lookup into O(1) instead of scanning the
    # alphabet string for every character.
